...
module cache_mem_set_associative #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (t_ren) random_way_mask <= random_way_mask_next;
`ifdef FORMAL
            // The way mask must be one hot or 0 at all times (must not get the
            // same tag in both ways of a set).
            if (t_addr_tag_prev != 0) assert (~|(way_mask & way_mask - 1));
`endif
        end
    end

    assign way_mask = way_mask_direct_en ? way_mask_direct : |hit_way ? hit_way : random_way_mask;
