...
module spi_regs (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (__csr_wen) csr_csauto_o <= csr_csauto_wdata;
            if (__csr_wen) csr_cs_o <= csr_cs_wdata;
            if (__csr_wen) csr_loopback_o <= csr_loopback_wdata;
            if (__csr_wen) csr_read_en_o <= csr_read_en_wdata;
            if (__csr_wen) csr_cpol_o <= csr_cpol_wdata;
            if (__csr_wen) csr_cpha_o <= csr_cpha_wdata;
            if (__div_wen) div_o <= div_wdata;
